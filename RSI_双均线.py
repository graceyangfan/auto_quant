...
class Strategy():
    ...
    def strategy_condition(self,short_period,long_period,RSI_threshold):
       ...
       # Log("start strategy_condition compute")
        ...
        if cross_below and rsi[-1] < 50 - RSI_threshold:
            trade_side = "sell"
        return trade_side 
    def make_trade_dicts(self, hands_num, short_period,long_period,RSI_threshold):
        ##资产，hands_num too high 无法交易 [ True,False]
        Asset_state = self.trade_amout_compute(hands_num)
        # 趋势判定是否买卖["buy","sell",False]
        Strategy_state = self.strategy_condition(short_period,long_period,RSI_threshold)
        if Asset_state:
...
def main():
    ...
    while(True):
        ...
        try:
            strategy.refresh_data(PERIOD_M5)
            now_trade_dicts = strategy.make_trade_dicts(args.hands_num,args.short_period,args.long_period,args.RSI_threshold)
            if now_trade_dicts:
