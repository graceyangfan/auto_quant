...
class Strategy():
    def __init__(self, args):
        ...
        self.position_change_percent = args.position_change_percent 
        self.donchian_channel_open_position = args.donchian_channel_open_position 
        self.donchian_channel_stop_profit = args.donchian_channel_stop_profit
    ...
    def recalc_paramter(self):
        self.n = TA.ATR(self.high_Arr,self.low_Arr,self.close_Arr,self.ATR_period)[-1]
        self.unit = int((self.total_Balance* self.position_change_percent) / (self.n))/self.quantcenter.Last
        self.donchian_channel_high = max(self.high_Arr[-self.donchian_channel_open_position - 1:-1])
        self.donchian_channel_low = min(self.low_Arr[-self.donchian_channel_open_position - 1:-1])
    def try_open(self):
        trade_price = self.quantcenter.Last 
        if self.status["position"] == 0 :
            if trade_price> self.donchian_channel_high:
                dic= self.create_order_dict("buy",trade_price,self.unit)
                if dic:
                    self.buy_orders.append(dic)
                    self.status["position"] = 1
                    self.status["last_trade_price"]  = dic["price"]
            elif  trade_price <self.donchian_channel_low:
                dic = self.create_order_dict("sell",trade_price,self.unit)
                if dic:
    ...
    def try_close(self):
        ...
        if self.status["position"] > 0 :
            ##后期改进仓位和风控
            if trade_price >= self.status["last_trade_price"]  + 0.5 * self.n :
                ##加仓
                dic= self.create_order_dict("buy",trade_price,self.unit)
                if dic:
                    self.buy_orders.append(dic)
                    self.status["position"] = 1
                    self.status["last_trade_price"]  = dic["price"]
            elif trade_price <= self.status["last_trade_price"]  -2.0 * self.n  or  \
                 trade_price <= min(self.low_Arr[-self.donchian_channel_stop_profit - 1:-1]) :
                ##止损 卖出所有
            ...
            if trade_price <= self.status["last_trade_price"]  - 0.5 * self.n :            
                ...
                if dic:
                 ...
                 trade_price >= max(self.high_Arr[-self.donchian_channel_stop_profit - 1:-1]):
                dic = self.create_order_dict("sell",trade_price,self.Amount)
                if dic:
                    self.sell_orders.append(dic)
                    self.status["position"] = 0
