...
class Strategy():
    ...
    def strategy_condition(self,short_period,long_period,signal_period,std_period,nbdev,moving_period):
       ...
       # Log("start strategy_condition compute")
        ...
        cross_over = dif[-2] > dea[-2] and dif[-3] < dea[-3]  and dif[-2] > 0 and  dea[-2] > 0  ##避免信号闪烁
        cross_below = dif[-2] < dea[-2] and dif[-3] > dea[-3] and  dif[-2] < 0 and  dea[-2] < 0
   
   
        #Log("rsi",rsi[-1])
        ##错误的点，仅仅考虑了价格和及其波动率没有考虑volume 
        trade_side = False
        if cross_over and rvi[-1] > self.RVI_threshold + 50:
            trade_side = "buy"
        if cross_below and rvi[-1] < 50 -self.RVI_threshold:
            trade_side = "sell"
        return trade_side 
    def make_trade_dicts(self,short_period,long_period,signal_period,std_period,nbdev,moving_period):
        # 趋势判定是否买卖["buy","sell",False]
        Strategy_state = self.strategy_condition(short_period,long_period,signal_period,std_period,nbdev,moving_period)
        #if Strategy_state:
    ...
    while(True):
        ...
        try:
            ...
            now_trade_dicts = strategy.make_trade_dicts(args.short_period,args.long_period,
                                                        args.signal_period,args.std_period,
                                                        args.nbdev,args.moving_period)
            if now_trade_dicts:
